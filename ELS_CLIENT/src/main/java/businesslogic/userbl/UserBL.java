...
public class UserBL implements UserBLService {
	...
	public UserBL() {
		try {
			udService = (UserDataService) Naming
					.lookup("rmi://localhost:8888/UserDataService");
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	}
	...
	public LogVO login(String userID, String password) {
		try {
			...
			if (userpo == null) // 找不到该用户，返回2
				...
				return new LogVO("The userID and the password don't match",
						null);
			else {
	...
	public static UserPO userVOToPO(UserVO uservo) {
		UserPO userpo = new UserPO(uservo.userName, uservo.userID,
				uservo.password, uservo.profession, uservo.organization,
				uservo.salaryPlan, uservo.authority, uservo.grades);
		return userpo;
	}
	...
	public static UserVO userPOToVO(UserPO userpo) {
		UserVO uservo = new UserVO(userpo.getName(), userpo.getID(),
				userpo.getPassword(), userpo.getProfession(),
				userpo.getOrganization(), userpo.getSalaryPlan(),
				userpo.getAuthority(), userpo.getGrades());
		return uservo;
	}
	...
	/*
	 * public static void main(String[] args){ try { UserDataService userData =
	 * (
	 * UserDataService)Naming.lookup("rmi://172.25.132.40:6000/UserDataService")
	 * ;
	 * 
	 * ArrayList<UserPO> userList0 = userData.showAllUsers(); for(UserPO
	 * user:userList0)
	 * System.out.println("ID: "+user.getID()+", Name: "+user.getName
	 * ()+", Profession: "+user.getProfession()+", Organization: "
	 * +user.getOrganization
	 * ()+", SalaryPlan: "+user.getSalaryPlan()+", Authority: "
	 * +user.getAuthority()+", Grades: "+user.getGrades());
	 * 
	 * userData.addUser(new UserPO("刘钦" ,"CK-00001","123456",
	 * ProfessionType.stockman, "南京中转中心", SalaryPlanType.basicStaffSalaryPlan,
	 * AuthorityType.lowest, 0));
	 * 
	 * ArrayList<UserPO> userList1 = userData.showAllUsers(); for(UserPO
	 * user:userList1)
	 * System.out.println("ID: "+user.getID()+", Name: "+user.getName
	 * ()+", Profession: "+user.getProfession()+", Organization: "
	 * +user.getOrganization
	 ...
	 * catch block e.printStackTrace(); } catch (NotBoundException e) { // TODO
