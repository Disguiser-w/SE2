 ...
 *
 * <p>
 * The calendar object is a <code>MouseListener</code> and a
 * <code>MouseMotionListener</code> for itself.
 * </p>
 *
 * <p>
 * User program shall not install this <code>MouseListener</code> or
 * <code>MouseMotionListener</code> to any other components; otherwise, runtime
 * exception will be thrown.
 * </p>
 *
 * <p>
 * All implementation of the <code>MouseListener</code> or <code>
 ...
 * empty implemented in this class.
 * </p>
 *
 * @author hysun
 */
public class MonthlyCalendar extends JComponent implements MouseListener, MouseMotionListener {
	...
	public static final String[] DEFAULT_HEADER_SYMBOLS = { "日", "一", "二", "三", "四", "五", "六" };

	/** Default font to be used */
	public static final Font DEFAULT_FONT = new Font("Microsoft YaHei", 0, 11);
	...
	protected final void paintComponent(Graphics g) {
		...
		if (headerComponent == null) {
			for (int i = 0; i < 7; i++) {
				...
				int y = 0;
				paintSymbol(g, headerFont, headerBg, headerFg, DEFAULT_HEADER_SYMBOLS[i], basex + x, basey + y,
		...
		if (year != -1 && month != -1) {
			for (int i = 0; i < 42; i++) {
				...
				if (dateData[i] == -1) {
					paintSymbol(g, dateFont, dateBg, dateFg, null, basex + x, basey + y, cellWidth, cellHeight);
				} else {
					if (hindex == i) {
						paintSymbol(g, dateFont, highBg, highFg, String.valueOf(dateData[i]), basex + x, basey + y,
								cellWidth, cellHeight);
					} else {
						paintSymbol(g, dateFont, dateBg, dateFg, String.valueOf(dateData[i]), basex + x, basey + y,
								cellWidth, cellHeight);
					}
				}
			}
